...
*   **`platform`**: Contains Android-specific implementations that rely on the Android SDK (e.g., `activity`, `application`, system services, intended `actual` declarations).
...
*   **Dependencies**: Pure Kotlin modules. No dependencies on Android framework or other layers (Data, Presentation, UI).
*   **Contents**:
...
*   **Mappers**: Format `SourceToDestinationMapper` (e.g., `BarcodeUiToDomainMapper`, `BarcodeDataToDomainMapper`).

## 4. Feature Directory Structure
Features are organized hierarchically within `presentation` and `ui` packages to keep related files together.
...
*   **Unidirectional Data Flow**:
    ...
    *   The **UI** triggers events (via function callbacks or an `handleUIEvent` lambda) which are processed by the `ViewModel`.
