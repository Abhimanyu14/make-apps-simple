...
package com.makeappssimple.abhimanyu.cosmos.design.system.components.chart.compose_pie
...
internal fun PieChartRenderer(
    ...
    Canvas(
        ...
        val drawInnerArc =
            sliceWidthPx > FLOAT_EPSILON && sliceWidthPx < chartSizePx / 2F
        ...
        finalFractions.fastForEachIndexed { index, sliceAngle ->
            ...
            val accountForSliceSpacing =
                sliceSpacingPx > 0F && sliceAngle <= 180F
            val sliceSpaceAngleOuter = sliceSpacingPx / (FDEG2RAD * outerRadius)
            val startAngleOuter =
                START_DEGREE + (angle + sliceSpaceAngleOuter / 2F) * phase
            val sweepAngleOuter = (sliceAngle - sliceSpaceAngleOuter) * phase
            pathBuffer.reset()
            val arcStartPointX =
                center.x + outerRadius * cos(startAngleOuter * FDEG2RAD)
            val arcStartPointY =
                center.y + outerRadius * sin(startAngleOuter * FDEG2RAD)
            ...
            if (drawInnerArc && (innerRadius > 0F || accountForSliceSpacing)) {
                ...
                val sliceSpaceAngleInner =
                    sliceSpacingPx / (FDEG2RAD * innerRadius)

                val startAngleInner =
                    START_DEGREE + (angle + sliceSpaceAngleInner / 2F) * phase
                val sweepAngleInner =
                    ((sliceAngle - sliceSpaceAngleInner) * phase).coerceAtLeast(
                        minimumValue = 0F,
                    )
                ...
                if (sweepAngleOuter % 360F > FLOAT_EPSILON) {
                    if (accountForSliceSpacing) {
                        ...
                        val arcEndPointX = center.x + sliceSpaceOffset * cos(
                            angleMiddle * FDEG2RAD
                        )
                        val arcEndPointY = center.y + sliceSpaceOffset * sin(
                            angleMiddle * FDEG2RAD
                        )
                        pathBuffer.lineTo(
                            x = arcEndPointX,
